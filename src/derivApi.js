...
export async function getAuthenticatedWsUrl({ appId, accessToken, accountId }) {
  if (!appId || !accessToken || !accountId) {
    throw new Error('Deriv account authentication is incomplete');
  }
  ...
  if (!response.ok || !data?.data?.url) {
    const message = data?.errors?.[0]?.message || data?.error || 'Unable to authenticate Deriv trading connection';
...
export class DerivConnection {
  ...
  async connect() {
    ...
    this.ws.onopen = () => {
      ...
      if (this.authenticated) {
        this.request({ balance: 1, subscribe: 1 });
      }
    ...
    this.ws.onclose = () => {
      this.authenticated = false;
      this.onStatus?.('Deriv connection closed');
    };
  ...
  subscribeTicks(symbol = 'R_100') {
    this.request({ ticks: symbol, subscribe: 1 });
  }

  send(payload) {
    if (!this.authenticated || this.ws?.readyState !== WebSocket.OPEN) {
      throw new Error('Authenticated Deriv trading connection is not ready');
    }
